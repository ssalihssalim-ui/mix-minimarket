...
// ✅ PAGINATION CORRIGÉE
// ✅ CORRECTION : closeCreditSelection ajoutée
...
function getPeriodOptions(selected) {
    ...
    return options.map(function(opt) {
        return '<option value="' + opt.value + '"' + (opt.value === selected ? ' selected' : '') + '>' + opt.label + '</option>';
    }).join('');
}

function filterByPeriod(data, period) {
    if (!data || period === 'all') return data.slice();
    ...
    return data.filter(function(item) {
        var itemSeconds = item.createdAt && item.createdAt.seconds ? item.createdAt.seconds : 0;
        return itemSeconds >= startSeconds;
    });
}

function applySort(collection, data, field) {
    if (!window.sortOrders) window.sortOrders = {};
    if (!window.sortOrders[collection]) window.sortOrders[collection] = {};
    
    var direction = window.sortOrders[collection][field] || 'desc';
...
// ✅ FONCTION closeCreditSelection - AJOUTÉE (correction de l'erreur)
...
console.log('✅ closeCreditSelection ajoutée - Erreur corrigée');
