...
// ✅ Support : POS, Produits, Crédits, Ventes
    ...
    if (voiceMode === 'search') {
        if ((currentPage === 'POS' || currentPage === 'Dashboard') && (window.posStep || 0) === 1) {
            ...
            if (products.length) {
                ...
                if (results.length > 0) {
                    if (results.length > 1) {
                        return { type: 'search_products', products: results, searchTerm: cleaned, page: 'pos' };
                    }
                    return { type: 'search_product', product: results[0], searchTerm: cleaned, page: 'pos' };
                }
            }
            ...
            if (bestAdmin) return { type: 'search_product', product: bestAdmin, searchTerm: cleaned, page: 'products' };
        }
    }
        ...
        case 'search_products':
            ...
            if (searchInput && cmd.products && cmd.products.length > 0) {
                ...
                setTimeout(function() {
                    ...
                    showVoiceResult('🔍 ' + foundCount + ' produit(s) trouvé(s) pour "' + searchTerm + '"');
                    setTimeout(function() {
        ...
        case 'search_product':
            ...
            if (cmd.page === 'products' || cp === 'Produits') {
                ...
                if (adminInput && cmd.product) {
                    ...
                    adminInput.value = searchTerm;
                    // ✅ Déclencher keyup pour appliquer le filtre
                    var event = new Event('keyup', { bubbles: true });
                    adminInput.dispatchEvent(event);
    ...
    voiceRecognition.onresult = function(e) {
        ...
        if (cp === 'Crédits') {
            ...
            if (vd && searchInput) {
                if (final) {
                    ...
                    vd.value = final;
                    // ✅ Déclencher l'événement input pour filtrer les crédits
                    var event = new Event('input', { bubbles: true });
                    searchInput.dispatchEvent(event);
        ...
        // ========== PAGE PRODUITS (ADMIN) ==========
            ...
            if (pi) {
                if (final) {
                    pi.value = final;
                    // ✅ Déclencher keyup pour appliquer le filtre sur la page produits
                    var event = new Event('keyup', { bubbles: true });
                    pi.dispatchEvent(event);
                    showProcessingIndicator();
                    var cmd = parseVoiceCommand(final);
                    if (cmd.type !== 'ignore') handleVoiceCommand(cmd);
                    hideVoiceFlowIndicator();
                    showVoiceResult('🔍 Recherche: ' + final);
                } else if (interim && interim !== lastInterim) {
                    pi.value = interim + ' ✍️';
                    lastInterim = interim;
                }
