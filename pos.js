...
// ✅ Bouton Retour 100% fonctionnel (onpointerdown + écouteur de secours)
...
// Sécurité : on rattache un écouteur addEventListener au cas où le navigateur ignorerait l'attribut inline
setTimeout(function() {
    var backBtn = document.querySelector('.pos-back-btn');
    if (backBtn) {
        // On nettoie tout ancien écouteur
        backBtn.removeEventListener('click', posBackHandler);
        backBtn.removeEventListener('pointerdown', posBackHandler);
        backBtn.removeEventListener('touchstart', posBackHandler);
        backBtn.removeEventListener('mousedown', posBackHandler);
        // On pose le nouvel écouteur
        backBtn.addEventListener('pointerdown', posBackHandler);
        backBtn.addEventListener('click', posBackHandler);
    }
}, 20);
...
// Fonction auxiliaire pour le bouton Retour
function posBackHandler(e) {
    e.preventDefault();
...
console.log('⚡ Mixmax Minimarket – POS chargé (bouton Retour OK)');
